...
class TextureHandlerImpl : TextureHandler {
    ...
    override fun findOrCreateTexture(label: String, forceReload: Boolean, content: ByteArray): Texture {
        return cache.takeIf { !forceReload } ?.computeIfAbsent(label) { createTexture0(content) }
                                             ?: createTexture0(content).also { cache[label] = it }
    }
    ...
    private fun createTexture0(path: String): Texture {
        ...
        return createTexture0(contentStream.readAllBytes())
    }

    private fun createTexture0(bytes: ByteArray): Texture {
        ...
        MemoryStack.stackPush().use {
            ...
            if (content == null) throw RuntimeException("Could not load image")
            width = w.get()
            height = h.get()
            channels = ch.get()
        }
