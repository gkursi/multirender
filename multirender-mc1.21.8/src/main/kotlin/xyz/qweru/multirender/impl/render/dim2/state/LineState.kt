...
class LineState(val pipeline: RenderPipeline, val texture: TextureSetup = TextureSetup.empty(), val pose: Matrix3x2f,
                val x1: Float, val y1: Float, val x2: Float, val y2: Float,
                val bounds: ScreenRect = MathUtils.screenRect(x1, y1, x2, y2).transformEachVertex(pose), val scissor: ScreenRect? = null,
                val color1: Int, val color2: Int = color1) : SimpleGuiElementRenderState {
    ...
    override fun setupVertices(b: VertexConsumer, depth: Float) {
        ...
        b.vertex(pose, start.x, start.y, depth).color(color1)
        setUV(b, 0f, 1f)
        b.normal(direction.x, direction.y, 0f)
        b.vertex(pose, start.x, start.y, depth).color(color2)
        setUV(b, 1f, 0f)
        b.normal(direction.x, direction.y, 0f)
    }
